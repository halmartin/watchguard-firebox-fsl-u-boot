...
void board_usb_cfg_reset(void)
{
	...
	if(is_t45cw()) /* Let T45-CW use the same USB configuration as T40-CW */
	{
		...
		puts("T45-CW: USB configuration for LTE/5G ... OK\n");
	}
#endif
}
...
int board_init(void)
{
	...
	#if defined(CONFIG_TARGET_LS1043AQDS_WG_T45)
	gpio_direction_output(324, 1); /* RTL8211F_PHYRSTB_1 */
	gpio_direction_output(325, 1); /* 8218E_RESET_1 */
	gpio_direction_output(327, 1); /* AW-XM458MA_W_DISABLE_1*/
	#endif
	...
	#if defined(CONFIG_TARGET_LS1043AQDS_WG_T45)
	puts("AW-XM458MA-PUR: Reset AW-XM458MA-PUR to fix wifi crash when soft reboot\n");
	gpio_direction_output(327, 0); /* AW-XM458MA_W_DISABLE_1*/
	udelay(150000); //keeping 150ms L
	gpio_direction_output(327, 1); /* AW-XM458MA_W_DISABLE_1*/
	#endif
	...
	#if defined(CONFIG_TARGET_LS1043AQDS_WG_T45)
	/* power sequence for RTL8211F */
	gpio_direction_output(324, 0); /* RTL8211F_PHYRSTB_1 */
	udelay(200000); //keeping 200ms
	gpio_direction_output(324, 1); /* RTL8211F_PHYRSTB_1 */
	/* power sequence for RTL8218E */
	gpio_direction_output(325, 0); /* 8218E_RESET_1 */
	udelay(20000); //keeping 20ms
	gpio_direction_output(325, 1); /* 8218E_RESET_1 */
	#endif
	...
	*/
	...
	/* Talor Lin: If model is T45-CW, then USB configuration should be changed for LTE/5G */
	board_usb_cfg_reset();
