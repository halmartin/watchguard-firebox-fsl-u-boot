...
#endif
{
	...
	#if defined(CONFIG_TARGET_LS1012AFRWY) || defined(CONFIG_TARGET_LS1012AFRWY_WG_NV5)
	if(strcmp(dev->name, "pfe_eth1") ==0 )
	{
		phydev = phy_device_create(bus, 0x3, PHY_FIXED_ID, interface);
	} else {
		phydev = phy_device_create(bus, 0x4, PHY_FIXED_ID, interface);
	}
	#else
